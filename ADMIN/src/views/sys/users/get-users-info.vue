    ...
    <a-form :model="state.form"
      ...
      <a-form-item label="头像"
        ...
        <div class="upload-box">
          <a-textarea v-model="state.form.avatar" allowClear placeholder="在线地址"></a-textarea>
          <a-image style="margin: 1rem 0" v-if="state.form.avatar" width="80px" :src="state.form.avatar"></a-image>
          <ZyUpload v-model="state.form.avatar"/>
        </div>
      </a-form-item>
      <a-form-item label="昵称"
                   name="nickname"
                   :rules="[{ required: true, message: '请输入昵称!' }]">
        <a-input v-model="state.form.nickname" allowClear placeholder="请输入昵称"/>
      </a-form-item>
      <a-form-item label="用户名"
                   name="username"
                   :rules="[{ required: true, message: '请输入用户名!' }]"
      >
        <a-input v-model="state.form.username" allowClear placeholder="请输入用户名"/>
      </a-form-item>
      ...
      >
        <a-input v-model="state.form.password" allowClear placeholder="请输入密码"/>
      </a-form-item>

      <a-form-item label="邮箱"
                   name="email">
        <a-input v-model="state.form.email" allowClear placeholder="请输入邮箱"/>
      </a-form-item>
      <a-form-item label="备注"
                   name="remark"
      >
        <a-textarea v-model="state.form.remark" allowClear placeholder="请输入备注"/>
      </a-form-item>
      <a-form-item label="角色" name="roleId"
                   :rules="[{ required: true, message: '请选择角色!' }]">
        <ZyRoleSelect v-model="state.form.roleId"/>
      </a-form-item>
    </a-form>

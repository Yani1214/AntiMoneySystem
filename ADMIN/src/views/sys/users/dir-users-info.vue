    ...
    <section>
      <ZySearchForm 
        ...
        <a-form-item name="username">
          <a-input v-model:value="state.query.params.username" allowClear placeholder="请输入用户名" @pressEnter="goPage" autocomplete="off" />
        </a-form-item>
      </ZySearchForm>
      <ZyFittleRow @add="goEdit" addAuth="sys:user:create" :showDelete="false" />
      <a-table
        bordered
        :resizable="true"
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'status'">
            <a-switch :checked="!!record.status" :disabled="!hasPerms('sys:users:update')" checked-children="正常" un-checked-children="停用" @change="statusChange(record)" />
          </template>
          ...
          <template v-else-if="column.key === 'action'">
            ...
            >
              <a-button type="primary" size="small" @click="resetPassword(record)" v-permission="'sys:users:reset'">重置密码 </a-button>
            </ZyToolButton>
          </template>
        </template>
      ...
      <ZyModal :minWidth="650" :show="state.show.edit" :title="state.editTitle" key="GetUserInfo" @close="close">
      ...
      <ZyModal :minWidth="650" :show="state.show.reset" title="重置密码" key="ResetUserInfo" @close="close">

...
import os
import shutil
from data_feature_7 import is_more_median,is_more_mean,is_more_qualite_25,is_more_qualite_75,time_interval,trans_frequency
#######################################################处理工行表格########################################################### 
"""
...
def batch_csv():
    # 给定目录路径
    dir = 'database/special/'
    dst_dir = 'database/special_people/'
...
"""
def feature_to_fill():
    people_folder = 'database/special_people'
...
def trans_to_fill():
# 设置文件夹路径
    folder_path = "database/special_people"
...
def id_to_fill():
    # 读取people.csv文件
    people_df = pd.read_csv("database/identity/combined_data.csv",dtype=str, encoding='gb18030')
    ...
    special_people_folder = "database/special_people"
...
def person_to_fill():
    ...
    # 读取CSV文件
    dst_file = 'database/identity/combined_data.csv'
    people_name = ['杨小琴', '苟兴兵', '何年碧', '何顺京', '胡秋艳', '李春荣', '李菊英', '孙翊章', '王庆凤', '颜爱中', '余英', '张柱碧']
...
def trans_to_people():
    src_directory = "database/special_people"
    dst_directory = "database/people"
    # 1. 收集某个目录下的所有文件名
    filenames = []
